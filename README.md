...
- **🤖 AI 动态终局**：内置 **Gemini AI**（通过 Cloudflare Worker 安全代理，无需学生手动输入 Key），在终局审判中与历史人物实时对话 — 你的辩护将决定角色的最终分数与历史评价
...
- **AI Engine**: Google Gemini API（通过 Cloudflare Worker 安全代理调用，Key 内置于云端）
...
### ⚡ AI 已内置，开箱即用
本项目 API 通过 **Cloudflare Worker** 安全代理调用 Gemini AI，Key 已内置在云端，学生**无需**手动输入任何密钥。打开游戏 → 页面自动检测模型 → 直接开玩！

## 📖 游戏流程
1. **设定玩家数量**并输入玩家名称
2. **每位玩家选择身份**：帝国军官 / 内政法官 / 反法同盟军
3. **经历关键历史抉择**，每个抉择影响角色的五大属性（军队、民心、财富、秩序、同盟）
4. **AI 终局审判**：在枫丹白露退位、维也纳会议等场景中，与历史人物对话，由 AI 评判你的结局（API 已内置，无需输入 Key）
